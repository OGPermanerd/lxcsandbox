...
- Delegate to $SCRIPT_DIR/02-create-container.sh then $SCRIPT_DIR/03-provision-container.sh
...
Run `./sandbox.sh create` with no args and verify usage message shows: "sandbox.sh create <name> <tailscale-key>"
Verify cmd_create function calls both `$SCRIPT_DIR/02-create-container.sh` and `$SCRIPT_DIR/03-provision-container.sh` by inspecting the code with grep.
  </verify>
  <done>
...
Run `./sandbox.sh badcommand` to verify it shows full usage (not just "run help").
  ...
  <done>
Help text updated, unknown commands show full usage.
  </done>
</task>
...
6. **Script delegation:** `grep -E 'SCRIPT_DIR.*(02-create|03-provision)' sandbox.sh` shows both delegations
